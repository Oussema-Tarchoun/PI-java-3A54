...
public class BackEnergieController implements Initializable {
    ...
    public void initialize(URL url, ResourceBundle rb) {
        ...
        for (User u : users) {
            userNamesMap.put(u.getId(), u.getNom());
        }
    ...
    private void loadData() {
        try {
            // Admin sees all energies in BackOffice
            allEnergies = serviceEnergie.recuperer();
            updateStats();
    ...
    private VBox buildCard(Energie e) {
        ...
        String uName = userNamesMap.getOrDefault(e.getUser_id(), "User " + e.getUser_id());
        Label userLabel = new Label("👤 " + uName);
        userLabel.setStyle("-fx-text-fill: #34d399; -fx-font-size: 11px; -fx-font-weight: bold; -fx-padding: 0 0 0 10;");
        
        typeRow.getChildren().addAll(typeLabel, idBadge, userLabel);
        ...
        btnEdit.setStyle("-fx-background-color: rgba(0,212,255,0.15); -fx-text-fill: #00d4ff; -fx-background-radius: 6; -fx-font-size: 12px; -fx-cursor: hand;");
        btnEdit.setOnAction(ev -> openEditForm(e));

        Button btnDel = new Button("✕");
        btnDel.setStyle("-fx-background-color: rgba(239,68,68,0.15); -fx-text-fill: #ef4444; -fx-background-radius: 6; -fx-font-size: 12px; -fx-cursor: hand;");
    ...
    private void handleSort() {
        ...
        handleSearch(); // Re-apply search/render with new order
    }
    ...
    private void handleCancel() {
        // No longer needed as integrated form is removed
    }
    ...
    private void handleDelete(Energie e) {
        ...
        alert.setContentText("Type: " + e.getType_energie() + "\nValeur: " + e.getValeur() + " kWh");
    ...
    private void generateAIRecommendation(Energie e) {
        ...
        new Thread(() -> {
            try {
                ...
                // Save or Update in database to make it persistent
                Recommandation reco = new Recommandation(
                    "Conseil IA - " + e.getType_energie(),
                    result,
                    "élevé", // IA recommendations are high priority
                    new java.util.Date(),
                    e.getId()
                );
                serviceRecommandation.modifier(reco); // modifier() in ServiceRecommandation adds if not exists
            } catch (Exception ex) {
                Platform.runLater(() -> aiTextArea.setText("❌ Impossible de générer la recommandation IA : " + ex.getMessage()));
                ex.printStackTrace();
            }
        }).start();
    }
