...
public class AlimentController implements Initializable {
    ...
    @FXML private TableColumn<Aliment, Integer>   colId;
    @FXML private TableColumn<Aliment, String>    colNom;
    @FXML private TableColumn<Aliment, Double>    colQuantite;
    @FXML private TableColumn<Aliment, Double>    colCalories;
    @FXML private TableColumn<Aliment, String>    colMacro;
    @FXML private TableColumn<Aliment, Void>      colActions;
    @FXML private Label     statTotal;
    @FXML private Label     statAvgCal;
    @FXML private Label     statMaxCal;
    @FXML private Label     statWithMacro;
    @FXML private Label     alimentCountLabel;
    ...
    @FXML private Button    btnTabRepas;
    @FXML private VBox      paneInfo;
    @FXML private VBox      paneRepas;

    /* ── Repas liés tab ── */
    @FXML private TableView<Repas>            repasLiesTable;
    ...
    @FXML private TableColumn<Repas, Integer> colRepasCal;
    ...
    public void setAliment(Aliment a) {
        ...
        if (pageTitle     != null) pageTitle.setText("Modifier : " + a.getNom());
        if (nomField      != null) nomField.setText(a.getNom());
        if (quantiteField != null) quantiteField.setText(formatDouble(a.getQuantite()));
        if (caloriesField != null) caloriesField.setText(formatDouble(a.getCalories()));
        String[] macros = parseMacro(a.getMacro());
        if (proteinesField != null) proteinesField.setText(macros[0]);
        if (glucidesField  != null) glucidesField.setText(macros[1]);
        if (lipidesField   != null) lipidesField.setText(macros[2]);
        loadRepasLies();
    }
    ...
    private void setupColumns() {
        ...
        colQuantite.setCellFactory(col -> new TableCell<>() {
            ...
            { lbl.getStyleClass().add("quantite-cell"); setAlignment(Pos.CENTER_LEFT); }
            @Override protected void updateItem(Double q, boolean empty) {
        ...
        colCalories.setCellFactory(col -> new TableCell<>() {
            private final Label lbl = new Label();
            { lbl.getStyleClass().add("calories-cell-double"); setAlignment(Pos.CENTER_LEFT); }
            @Override protected void updateItem(Double c, boolean empty) {
        ...
        colMacro.setCellValueFactory(d ->
                new javafx.beans.property.SimpleStringProperty(d.getValue().getMacro()));
        colMacro.setCellFactory(col -> new TableCell<>() {
            private final HBox box = new HBox(6);
            { box.setAlignment(Pos.CENTER_LEFT); }
            @Override protected void updateItem(String macro, boolean empty) {
    ...
    private void updateStats() {
        ...
        if (alimentCountLabel != null) alimentCountLabel.setText(total + " aliments au total");
        if (total > 0) {
            double avg = allAliments.stream()
                    .mapToDouble(Aliment::getCalories).average().orElse(0);
            if (statAvgCal != null) statAvgCal.setText(formatDouble(avg) + " kcal");
            ...
            if (statAvgCal    != null) statAvgCal.setText("0 kcal");
    ...
    @FXML private void openAddAliment() {
    ...
    private void handleCreate() {
        ...
        if (nom.isBlank()) { showError("Le nom de l'aliment est obligatoire."); return; }
        if (nom.trim().length() < 2) { showError("Le nom doit contenir au moins 2 caractères."); return; }
        ...
        if (caloriesField != null && !caloriesField.getText().isBlank()) {
            ...
            if (Double.isNaN(manual)) return;
            if (manual > 0) calories = manual;
        }

        String macro = String.format("P:%.1f;G:%.1f;L:%.1f", proteines, glucides, lipides);
        Aliment aliment = new Aliment(nom.trim(), quantite, calories, macro);
        try {
    ...
    @FXML private void showInfoTab() {
        ...
        if (paneRepas != null) { paneRepas.setVisible(false); paneRepas.setManaged(false); }
        if (btnTabInfo  != null) btnTabInfo.getStyleClass().setAll("form-tab-btn", "active");
        if (btnTabRepas != null) btnTabRepas.getStyleClass().setAll("form-tab-btn");
    }

    @FXML private void showRepasTab() {
        if (paneRepas != null) { paneRepas.setVisible(true);  paneRepas.setManaged(true);  }
        if (paneInfo  != null) { paneInfo.setVisible(false);  paneInfo.setManaged(false);  }
        if (btnTabRepas != null) btnTabRepas.getStyleClass().setAll("form-tab-btn", "active");
    ...
    private void handleUpdate() {
        ...
        if (nom.trim().length() < 2) { showError("Le nom doit avoir au moins 2 caractères."); return; }
    ...
    @FXML private void handleDelete() {
        if (currentAliment != null) confirmAndDelete(currentAliment);
    }

    private void handleDelete(Aliment a) { confirmAndDelete(a); }
    ...
    private void setupRepasLiesColumns() {
        if (repasLiesTable == null) return;
        colRepasId.setCellValueFactory(d ->
                new javafx.beans.property.SimpleIntegerProperty(
                        d.getValue().getId()).asObject());
        colRepasNom.setCellValueFactory(d ->
                new javafx.beans.property.SimpleStringProperty(d.getValue().getNom()));
        colRepasType.setCellValueFactory(d ->
                new javafx.beans.property.SimpleStringProperty(d.getValue().getType()));
        colRepasDate.setCellValueFactory(d ->
                new javafx.beans.property.SimpleStringProperty(
                        d.getValue().getDate() != null ? d.getValue().getDate().toString() : ""));
        colRepasCal.setCellValueFactory(d ->
                new javafx.beans.property.SimpleIntegerProperty(
                        d.getValue().getCalories()).asObject());
    }

    private void loadRepasLies() {
        if (repasLiesTable == null || currentAliment == null) return;
        try {
    ...
    /* ══════════════════════════════════════════════
       NAVIGATION
    ══════════════════════════════════════════════ */
    @FXML private void goBack()       { navigateTo("/view/aliments/ListAliment.fxml"); }
    ...
    @FXML private void goToRepas()      { navigateTo("/view/repas/ListRepas.fxml"); }
