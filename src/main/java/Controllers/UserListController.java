...
public class UserListController {
    ...
    @FXML
    private TableColumn<User, String> colBlocked;
    @FXML
    private TableColumn<User, String> col2FA;
    @FXML
    private TableColumn<User, String> colXP;
    @FXML
    ...
    private void setupColumns() {
        ...
        colRoles.setCellValueFactory(c -> {
            ...
            if (r.contains("ROLE_ADMIN"))
                return new SimpleStringProperty("🔑 Admin");
            if (r.contains("ROLE_MODERATOR"))
                return new SimpleStringProperty("🛡 Modérateur");
            return new SimpleStringProperty("👤 Utilisateur");
        });
        colVerified
                .setCellValueFactory(c -> new SimpleStringProperty(c.getValue().getIsVerified() ? "✅ Oui" : "❌ Non"));
        colBlocked.setCellValueFactory(
                c -> new SimpleStringProperty(c.getValue().getIsBlocked() == 1 ? "🔒 Bloqué" : "✔ Actif"));
        col2FA.setCellValueFactory(c -> new SimpleStringProperty(c.getValue().getIs2faEnabled() ? "🔐 Oui" : "Non"));
        colXP.setCellValueFactory(c -> new SimpleStringProperty(String.valueOf(c.getValue().getExperiencePoints())));
    }
        ...
        try {
            ...
            Paragraph sub = new Paragraph(
                    "Exporté le : " + java.time.LocalDate.now()
                            + "   |   Total : " + usersToExport.size() + " utilisateur(s)",
                    subFont);
            ...
            for (User u : usersToExport) {
                ...
                if (u.getRoles() != null) {
                    if (u.getRoles().contains("ROLE_ADMIN"))
                        role = "Admin";
                    else if (u.getRoles().contains("ROLE_MODERATOR"))
                        role = "Modérateur";
                }
    ...
    private void goToDashboard() {
        if (MainController.instance != null) {
            MainController.instance.openDashboard();
        }
    }
    ...
    private void openReportManagement() {
        if (MainController.instance != null) {
            MainController.instance.openGlobalReports();
        }
    }
    ...
    private void openUserProfile(User user) {
        try {
            ...
            if (MainController.instance != null) {
                MainController.instance.setContent(root, "Profil Utilisateur", user.getName());
            } else if (Controllers.repas.RepasController.instance != null) {
                Controllers.repas.RepasController.instance.loadInCenter(root);
            } else {
