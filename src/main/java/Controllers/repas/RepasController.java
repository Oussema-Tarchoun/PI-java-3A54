...
public class RepasController implements Initializable {
    ...
    @FXML
    private TableColumn<Repas, Integer> colId;
    @FXML
    private TableColumn<Repas, String> colNom;
    @FXML
    private TableColumn<Repas, String> colType;
    @FXML
    private TableColumn<Repas, String> colDate;
    @FXML
    private TableColumn<Repas, String> colHeure;
    @FXML
    private TableColumn<Repas, Integer> colCalories;
    @FXML
    private TableColumn<Repas, String> colScore;
    @FXML
    private TableColumn<Repas, Void> colActions;
    @FXML
    private Label statTotalRepas;
    @FXML
    private Label statAvgCalories;
    @FXML
    private Label statCeMois;
    @FXML
    private Label statScore;
    @FXML
    private Label repasCountLabel;
    @FXML
    ...
    @FXML
    private javafx.scene.layout.BorderPane rootBorderPane;
    @FXML
    private TableColumn<Repas, Void> colPdf;

    @FXML
    private VBox mainContentArea;
    private javafx.scene.Node originalContent;
    ...
    @FXML
    private Button btnTabAliments;
    @FXML
    private VBox paneInfo;
    @FXML
    private VBox paneAliments;
    @FXML
    private ListView<Aliment> alimentsListView;
    @FXML
    private Label errorLabelAliments;
     ...
     * ══════════════════════════════════════════════
     */
    public static RepasController instance;

    @Override
    public void initialize(URL url, ResourceBundle rb) {
        instance = this;
        originalContent = mainContentArea;
        ...
        if (alimentsListView != null) {
            ...
            alimentsListView.setCellFactory(lv -> new ListCell<>() {
                @Override
                protected void updateItem(Aliment a, boolean empty) {
    ...
    public void setRepas(Repas r) {
        this.currentRepas = r;
        if (pageTitle != null)
            pageTitle.setText("Modifier : " + r.getNom());
        if (nomField != null)
    ...
    private void setupColumns() {
        ...
        colHeure.setCellValueFactory(d -> new javafx.beans.property.SimpleStringProperty(
        ...
        colCalories.setCellValueFactory(d -> new javafx.beans.property.SimpleIntegerProperty(
                d.getValue().getCalories()).asObject());
        colCalories.setCellFactory(col -> new TableCell<>() {
            private final Label lbl = new Label();
            {
                lbl.getStyleClass().add("calories-cell");
                setAlignment(Pos.CENTER_LEFT);
            }

            @Override
            protected void updateItem(Integer kcal, boolean empty) {
    ...
    private void exportPdf(Repas r) {
        try {
            ...
            String path = System.getProperty("user.home") + "/Desktop/repas_" + r.getId() + "_"
                    + r.getNom().replaceAll("\\s+", "_") + ".pdf";
    ...
    private void updateStats() {
        ...
        if (statTotalRepas != null)
            statTotalRepas.setText(String.valueOf(total));
        if (repasCountLabel != null)
            repasCountLabel.setText(total + " repas au total");
        if (total > 0) {
            double avg = allRepas.stream().mapToInt(Repas::getCalories).average().orElse(0);
            if (statAvgCalories != null)
                statAvgCalories.setText((int) avg + " kcal");
            ...
            if (statScore != null)
                statScore.setText("Excellent");
        } else {
            if (statAvgCalories != null)
                statAvgCalories.setText("0 kcal");
            if (statCeMois != null)
    ...
    private void setupSearch() {
        ...
        searchField.textProperty().addListener((obs, o, newVal) -> {
            if (newVal == null || newVal.isBlank()) {
                ...
                repasTable.setItems(allRepas.filtered(r -> r.getNom().toLowerCase().contains(lower) ||
    ...
    private void sortAscCalories() {
        if (repasTable != null)
            repasTable.getItems().sort((a, b) -> Integer.compare(a.getCalories(), b.getCalories()));
    }

    /**
     * Loads an FXML into the center of the root BorderPane, keeping the sidebar.
     */
    public void loadInCenter(String fxmlPath) {
        try {
            javafx.scene.Parent center = FXMLLoader.load(getClass().getResource(fxmlPath));
            loadInCenter(center);
        } catch (Exception e) {
    ...
    public void loadInCenter(javafx.scene.Parent center) {
        if (rootBorderPane != null) {
            rootBorderPane.setCenter(center);
        } else {
            // Fallback strategy: try replacing the scene root
            javafx.scene.Node anchor = repasTable != null ? repasTable : nomField != null ? nomField : pageTitle;
            if (anchor != null && anchor.getScene() != null) {
                anchor.getScene().setRoot(center);
            }
        }
    }
    ...
    private void sortDescCalories() {
        if (repasTable != null)
            repasTable.getItems().sort((a, b) -> Integer.compare(b.getCalories(), a.getCalories()));
    }

    @FXML
    private void openAddRepas() {
        navigateTo("/view/repas/AddRepas.fxml");
    }
    ...
    private void handleCreate() {
        ...
        if (nom.isBlank()) {
            showError("Le nom du repas est obligatoire.");
            return;
        }
        ...
        if (heureStr.isBlank()) {
            showError("L'heure est obligatoire (ex: 12:30:00).");
            return;
        }
        ...
        try {
            ...
            showError("Format d'heure invalide. Utilisez HH:mm ou HH:mm:ss.");
            return;
        }
        ...
        if (!calStr.isBlank()) {
            try {
                ...
                showError("Les calories doivent être un entier.");
                return;
            }
        }
    ...
    private void showInfoTab() {
        ...
        if (paneAliments != null) {
            paneAliments.setVisible(false);
            paneAliments.setManaged(false);
        }
        if (btnTabInfo != null)
            btnTabInfo.getStyleClass().setAll("form-tab-btn", "active");
        if (btnTabAliments != null)
            btnTabAliments.getStyleClass().setAll("form-tab-btn");
    }

    @FXML
    private void showAlimentsTab() {
        if (paneAliments != null) {
            paneAliments.setVisible(true);
            paneAliments.setManaged(true);
        }
        if (paneInfo != null) {
        ...
        if (btnTabAliments != null)
            btnTabAliments.getStyleClass().setAll("form-tab-btn", "active");
        if (btnTabInfo != null)
    ...
    private void handleDelete() {
        if (currentRepas != null)
            confirmAndDelete(currentRepas);
    }
    ...
    private void loadAlimentsList() {
        if (alimentsListView == null || currentRepas == null)
            return;
        try {
            List<Aliment> all = serviceAliment.recuperer();
    ...
    private void goToRepas() {
        if (originalContent != null && rootBorderPane != null) {
            rootBorderPane.setCenter(originalContent);
        } else {
            navigateTo("/view/repas/ListRepas.fxml");
        }
    }
    ...
    @FXML
    private void goToGlobalReports() {
        loadInCenter("/GlobalReportList.fxml");
    }

    @FXML
    private void openUserManagement() {
        loadInCenter("/UserList.fxml");
    }

    @FXML
    private void goToAliments() {
        loadInCenter("/view/aliments/ListAliment.fxml");
    }
