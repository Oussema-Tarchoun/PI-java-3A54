...
public class UserProfileController {
    ...
    @FXML
    private Label lblResetAttempts;
    @FXML
    private ProgressIndicator progressXP;
    @FXML
    ...
    private void populateData() {
        ...
        if (roles != null) {
            if (roles.contains("ROLE_ADMIN")) {
                ...
                lblRoleBadge.setStyle(
                        "-fx-background-color: #6c63ff; -fx-text-fill: white; -fx-padding: 4 12; -fx-background-radius: 20;");
            } else if (roles.contains("ROLE_MODERATOR")) {
                lblRoleBadge.setText("MODÉRATEUR");
                lblRoleBadge.setStyle(
                        "-fx-background-color: #f59e0b; -fx-text-fill: white; -fx-padding: 4 12; -fx-background-radius: 20;");
            } else {
                lblRoleBadge.setText("UTILISATEUR");
                lblRoleBadge.setStyle(
                        "-fx-background-color: #373a50; -fx-text-fill: white; -fx-padding: 4 12; -fx-background-radius: 20;");
            }
        }
        ...
        lblVerified.setStyle(
                user.getIsVerified() ? "-fx-text-fill: #6c63ff; -fx-font-weight: bold;" : "-fx-text-fill: #9ca3b0;");
    ...
    private void goBack() {
        try {
            ...
            if (MainController.instance != null) {
                MainController.instance.setContent(root, "AIVA Admin — Gestion des Utilisateurs", "");
    ...
    private void handleExport() {
        ...
        try {
            ...
            addTableCell(table, "POINTS XP", String.valueOf(user.getExperiencePoints()), BaseColor.WHITE, labelFont,
                    valueFont);
    ...
    private void handleManageReports() {
        try {
            ...
            if (MainController.instance != null) {
                MainController.instance.setContent(root, "Gérer les Rapports", user.getName());
            } else {
                Stage stage = (Stage) lblName.getScene().getWindow();
                stage.getScene().setRoot(root);
