...
public class EmailService {
    ...
    public void sendRapportNutrition(String toEmail, String userName,
        ...
        html.append("<div style='font-family:Arial,sans-serif;max-width:600px;margin:auto;background:#0a0e1a;color:#fff;border-radius:12px;overflow:hidden;'>")
                ...
                .append("<p>Bonjour <strong>").append(userName).append("</strong>, voici votre rapport nutritionnel hebdomadaire.</p>")
        ...
        data.get("points_positifs").forEach(p -> html.append("<li style='color:#cbd5e1;margin:4px 0;'>").append(p.asText()).append("</li>"));
        html.append("</ul>");

        // À améliorer
        html.append("<h3 style='color:#fbbf24;'>⚠️ À améliorer</h3><ul>");
        data.get("a_ameliorer").forEach(p -> html.append("<li style='color:#cbd5e1;margin:4px 0;'>").append(p.asText()).append("</li>"));
        html.append("</ul>");

        // Analyse par jour
        html.append("<h3 style='color:#93c5fd;'>📅 Analyse par jour</h3>");
        data.get("analyse_par_jour").forEach(j ->
                html.append("<div style='background:#161b2e;padding:10px 14px;border-radius:6px;margin:6px 0;'>")
        ...
        data.get("conseils").forEach(p -> html.append("<li style='color:#cbd5e1;margin:4px 0;'>").append(p.asText()).append("</li>"));
        html.append("</ul>")
                .append("</div>")
