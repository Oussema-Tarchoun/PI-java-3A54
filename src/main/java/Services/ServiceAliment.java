...
public class ServiceAliment implements Iservice<Aliment> {
    ...
    public void modifier(Aliment aliment) throws SQLDataException {
        String sql = "UPDATE aliment SET user_id=?, nom=?, quantite=?, calories=?, macro=? WHERE id=?";
        try (PreparedStatement ps = getConnection().prepareStatement(sql)) {
            ps.setInt(1, aliment.getUser_id());
            ps.setString(2, aliment.getNom());
            ps.setDouble(3, aliment.getQuantite());
            ps.setDouble(4, aliment.getCalories());
            ps.setString(5, aliment.getMacro());
            ps.setInt(6, aliment.getId());
            ps.executeUpdate();
        } catch (Exception e) {
            System.out.println(e.getMessage());
        }
    ...
    public void ajouter(Aliment aliment) throws SQLDataException {
        String sql = "INSERT INTO aliment (user_id, nom, quantite, calories, macro) VALUES (?, ?, ?, ?, ?)";
        try (PreparedStatement ps = getConnection().prepareStatement(sql)) {
            ps.setInt(1, aliment.getUser_id());
            ps.setString(2, aliment.getNom());
            ps.setDouble(3, aliment.getQuantite());
    ...
    public List<Aliment> recupererParUser(int userId) throws SQLDataException {
        List<Aliment> list = new ArrayList<>();
        String sql = "SELECT * FROM aliment WHERE user_id = ?";
        try (PreparedStatement ps = getConnection().prepareStatement(sql)) {
            ps.setInt(1, userId);
            ...
            System.out.println("recupererParUser aliment error: " + e.getMessage());
        }
        return list;
    }
    ...
    private String estimateMacro(String name, int calories) {
        ...
        if (n.matches(".*(poulet|dinde|thon|saumon|boeuf|veau|porc|crevette|moule|sardine|cabillaud|oeuf).*")) {
            ...
            l = calories * 0.92 / 9;
                ...
                ".*(pain|riz|pate|spaghetti|farine|pomme de terre|couscous|quinoa|sucre|miel|avoine|biscuit).*")) {
            p = calories * 0.10 / 4;
            g = calories * 0.78 / 4;
            l = calories * 0.12 / 9;
            ...
            l = calories * 0.47 / 9;
                ...
                ".*(tomate|salade|epinard|carotte|brocoli|courgette|poivron|oignon|pomme|banane|fraise|orange|citron|datte|raisin|mangue).*")) {
            p = calories * 0.12 / 4;
            g = calories * 0.72 / 4;
            l = calories * 0.16 / 9;
    ...
    public Aliment findByNameOrCreate(int userId, String name, int calories, String quantity) {
        String searchSql = "SELECT * FROM aliment WHERE user_id = ? AND LOWER(nom) = LOWER(?) LIMIT 1";
            ...
            if (rs.next()) {
                ...
                if ((existingCal == 0 && calories > 0) || (existingMacro == null || existingMacro.isBlank())) {
                    ...
                    String newMacro = (existingMacro == null || existingMacro.isBlank())
                            ? estimateMacro(name.trim(), (int) newCal)
                            : existingMacro;
        ...
        String insertSql = "INSERT INTO aliment (user_id, nom, quantite, calories, macro) VALUES (?, ?, ?, ?, ?)";
        try (PreparedStatement ps = getConnection().prepareStatement(insertSql, Statement.RETURN_GENERATED_KEYS)) {
            double qty = 100;
            try {
                String num = quantity.replaceAll("[^0-9.]", "");
                if (!num.isEmpty())
            ...
            ps.setString(5, estimateMacro(name.trim(), calories));
            ps.executeUpdate();
